                                ...
                                if(answer.first == -1){
                                        if(iflag){
                                                if(fctb_found_pos == -10){
                                                        ...
                                                        if(pa == 0){
                                                                v2p = va_to_pa(read_cpu, RQ.entry[index].instr_id, RQ.entry[index].full_addr,
           RQ.entry[index].address, RQ.entry[index].ip, RQ.entry[index].type, iflag, 0); pa = v2p.first;
                                                        }
                                                }
                                        }
                                        else{
                                                v2p = va_to_pa(read_cpu, RQ.entry[index].instr_id, RQ.entry[index].full_addr, RQ.entry[index].address,
           RQ.entry[index].ip, RQ.entry[index].type, iflag, 0); pa  = v2p.first;
                                        }
                                        if (iflag == 1 || DSTLB == 1)
                                                pf_misses_pq++;
                                }
                                ...
                                if(iflag == 1){
                                        free_indexes = sorted_free_distances();
                                        stlb_prefetcher_operate(RQ.entry[index].address, RQ.entry[index].ip, 0, RQ.entry[index].type, answer.first,
           warmup_complete[cpu], free_indexes, RQ.entry[index].instr_id, iflag); stlb_prefetcher_cache_fill(RQ.entry[index].address, 0, 0, 0, 0);
                                }

                                //Added by Munawira to support Data + Inst Prefetcher at STLB
                                //dstlb_prefetcher_operate(RQ.entry[index].address, RQ.entry[index].ip, 0, RQ.entry[index].type, answer.first,
           warmup_complete[cpu], free_indexes, RQ.entry[index].instr_id, iflag);
