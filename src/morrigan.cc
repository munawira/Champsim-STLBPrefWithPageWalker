...
void show_s4()
{
  ...
  for (int i = 0; i < x; ++i) {
    for (int j = 0; j < PT_S4_ASSOC; ++j) {
      cout << hex << irip_s4[i][j].vpn << ", " << irip_s4[i][j].successor_delta[0] << ", " << irip_s4[i][j].successor_delta[1] << ", "
           << irip_s4[i][j].successor_delta[2] << ", " << irip_s4[i][j].successor_delta[3] << " | ";
    }
    cout << endl;
  }
}
...
void show_s8()
{
  ...
  for (int i = 0; i < x; ++i) {
    for (int j = 0; j < PT_S8_ASSOC; ++j) {
      cout << hex << irip_s8[i][j].vpn << ", " << irip_s8[i][j].successor_delta[0] << ", " << irip_s8[i][j].successor_delta[1] << ", "
           << irip_s8[i][j].successor_delta[2] << ", " << irip_s8[i][j].successor_delta[3] << ", " << irip_s8[i][j].successor_delta[4] << ", "
           << irip_s8[i][j].successor_delta[5] << ", " << irip_s8[i][j].successor_delta[6] << ", " << irip_s8[i][j].successor_delta[7] << " | ";
    }
    cout << endl;
  }
}
...
void CACHE::stlb_prefetcher_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, uint8_t type, int answer, int warmup, int* free_indexes, uint64_t instr_id,
                                    int iflag)
{
  if (!iflag)
    assert(0);
  ...
  // MUNA: Below code updates the confidence counter of any entry in the PT_ST distance that matches the address
  // Not edited by me, copied as is from original Morrigan code
  if (answer != -1) {
  ...
  // MUNA: Search all PT_Sx tables and find a hit
  ...
  // MUNA: If not found in any table :
  if (way == -1) {
    int fi_lim = -100;
    ...
    if (answer == -1 && ENABLE_FP == 1)
      ignore = 1;
    else { // MUNA: If found/not found in prefetch queue? prefetch for all entries fron 0 to 7
      int free = 0;
      if (answer == -1)
        free = 1;
...
void CACHE::stlb_prefetcher_final_stats(uint64_t prefetches, uint64_t hits, uint64_t misses, uint64_t swap, uint64_t dupli, uint64_t free, uint64_t real,
                                        uint64_t* mmu_cache_demand_hits, uint64_t* mmu_cache_prefetch_hits, uint64_t* rfhits, uint64_t* free_hits,
                                        uint64_t mr[4][4], uint64_t stlb_misses[2])
{
  cout << endl << "*******************************************" << endl;
