...
void add_stall_prefetch(int cycles, uint32_t cpu) { stall_cycle[cpu] = current_core_cycle[cpu] + cycles; }
...
int mmu_cache_prefetch_search(uint32_t cpu, uint64_t vpage, int swap, uint64_t instr_id, uint64_t ip, int type, int iflag)
{
  ...
  if (swap == 0) {
    ...
    if (LOG2_PAGE_SIZE == 12) {
      ...
      pdp2s = cr3 + 512 * 8 + pml4_index * 512 * 8 + pdp_index * 8;

      pd2s = cr3 + 512 * 8 + 512 * 512 * 8 + pml4_index * 512 * 512 * 8 + pdp_index * 512 * 8 + pd_index * 8;

      pt2s = cr3 + 512 * 8 + 512 * 512 * 8 + 512 * 512 * 512 * 8 + pml4_index * 512 * 512 * 512 * 8 + pdp_index * 512 * 512 * 8 + pd_index * 512 * 8
             + pt_index * 8;
    } else {
      pml42s = cr3 + pml4_index * 8;
      ...
      pt2s = cr3 + 512 * 8 + 512 * 512 * 8 + pml4_index * 512 * 512 * 8 + pdp_index * 512 * 8 + pt_index * 8;
    }
